...
const LoadTestDetailsPage = () => {
  ...
  const { data: selectedRun } = useLoadTestRun(selectedRunId || '');

  // Fetch reports for selected run
  const { data: reports } = useLoadTestReports(selectedRunId || '');

  // Fetch selected report with results
  const { data: selectedReport } = useLoadTestReport(selectedReportId || '', currentPage, pageSize);

  const runs = config?.runs || [];
  const selectedReportResults = selectedReport?.results || [];
        ...
        <FadeIn>
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="sm" onClick={() => navigate('/load-tests')}>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium truncate">{config.webhook?.url || 'N/A'}</p>
                </div>
                <div>
                ...
                {config.webhook?.headers && Object.keys(config.webhook.headers).length > 0 && (
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      {Object.keys(config.webhook.headers).length} header(s)
                    </p>
                  </div>
                )}
                {config.webhook?.query_params &&
                  Object.keys(config.webhook.query_params).length > 0 && (
                    <div>
                      ...
                      <p className="text-sm font-medium">
                        {Object.keys(config.webhook.query_params).length} param(s)
                      </p>
                    </div>
                  )}
                {config.webhook?.body_template && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Has Body</p>
                    <Badge variant="outline" className="mt-1">
                      Yes
                    </Badge>
                  </div>
                ...
                >
                  ...
                  {reports && reports.length > 0 ? (
                            ...
                            <CardContent className="p-3">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                  <span>{report.total_requests} req</span>
                                  <span className="text-green-500">
                                    {report.successful_requests} ✓
                                  </span>
                                  {report.avg_response_time_ms && (
                                    <span>{report.avg_response_time_ms}ms</span>
                                  )}
                                </div>
                              </div>
                            </CardContent>
                    ...
                    <Card className="rounded-xl border-border/50">
                      <CardContent className="p-8 text-center">
                        <p className="text-sm text-muted-foreground">
                          No reports available for this run.
                        </p>
                      </CardContent>
                    </Card>
                  )}
